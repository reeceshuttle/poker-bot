...
class Player(Bot):
    ...
    def handle_new_round(self, game_state, round_state, active):
        ...
        self.hand_num+=1
        self.preflop_bet = 1
        self.preflop_aggressor = True # is True or False
    ...
    def get_action(self, game_state, round_state, active):
        ...
        if street == 0: # if we are pre-flop
            if self.my_bankroll>(NUM_ROUNDS-self.hand_num+1)*1.5+2: # if win is secured, check fold
                ...
                return preflop_strategy.LimpStrategy(game_state, round_state, active)
            elif my_pip == 2 and opp_pip > 2: # they raised from the btn, implement defense strategy
                return preflop_strategy.DefenseStrategy(game_state, round_state, active)
            elif my_pip > 2 and opp_pip > 2: # they 3 bet my raise or limp 3bet
                return preflop_strategy.Defend3betStrategy(game_state, round_state, active) # this is really the nth bet defense now.
        ...
        # ----------- post-flop strategy ------------
            ...
            if self.my_bankroll>(NUM_ROUNDS-self.hand_num+1)*1.5+2:
                ...
                if active == 1: # means we start betting round
                    ...
                    return postflop_strategy.CheckedToStrategy(game_state, round_state, active)
            
            elif my_pip == 0 and opp_pip > 0: # They bet first, meaning they start or I checked to them
                if active == 1: # I checked to them, they bet
                    return postflop_strategy.ICheckedTheyBetStrategy(game_state, round_state, active)
